...
from src.models.linear_regressor import LinearRegressorModel
...
def main():
    ...
    ###########################################
        ...
        lin_reg.train(X_train_scaled, y_train)
        lin_reg_metrics = lin_reg.evaluate(X_test_scaled, y_test)
        
        # --- Plots do Linear ---
        lin_predictions = lin_reg.predict(X_test_scaled)
        reg_viz.plot_predictions_vs_real(y_test, lin_predictions, model_name='Linear')
        reg_viz.plot_error_distribution(y_test, lin_predictions, model_name='Linear')
        reg_viz.plot_metrics(lin_reg_metrics, model_name='Linear')
        
        print("\n--- Gerando Gráfico de Features (Linear Regressor) ---")
        importances_df_linear = lin_reg.get_feature_importances(feature_names)
        reg_viz.plot_feature_importance(importances_df_linear, model_name='Linear_Regressor')
